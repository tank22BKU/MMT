...
class MyClient(Client):
    ...
    def Client_Process(self, fileName, peerNum, serverIPs, serverPorts, chunkNum):
        logs = []
        st.text(f"Đang tải {fileName} từ ID:{peerNum} peer")
...
if selected_tab == "Client":
    ...
    with col2:
        ...
        if submit_button:
            ...
            if serverName[0] == "None":   
                serverName[0] = "192.168.88.52"
            
            st.write("File name:", {fileName})
            st.write("Tracker URL:", {tracker_url})
            st.write("Chunk number:", {chunkNum})
            st.write("Server name:", serverName)
            st.write("Server port:", serverPort)
            st.write("Peer number:", {peerNum})
